...
import { useUpdatePaymentPolicy } from "@/hooks/policies/paymentPolicies/useUpdatePaymentPolicy";
import { usePaymentPolicy } from "@/hooks/policies/paymentPolicies/usePaymentPolicy";
...
function UpdatePaymentPolicy() {
  ...
  const { isLoading: isLoadingPaymentPolicy, paymentPolicy } =
    usePaymentPolicy();
  const { isLoading: isUpdating, updatePolicy } = useUpdatePaymentPolicy();
  ...
  async function onSubmit({}, e) {
    ...
    updatePolicy({ policyId: paymentPolicy.id, data: { content, visible } });
  }
  ...
  async function handleCancel() {
    setContent(paymentPolicy?.content);
    setVisible(paymentPolicy?.visible);
  }

  if (isLoadingPaymentPolicy) return <Spinner />;

  return (
    <>
      <Row type="horizontal">
        <Heading as="h1">Cập nhật chính sách thanh toán</Heading>
...
export default UpdatePaymentPolicy;
