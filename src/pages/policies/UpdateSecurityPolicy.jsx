...
import { useUpdateSecurityPolicy } from "@/hooks/policies/securityPolicies/useUpdateSecurityPolicy";
import { useSecurityPolicy } from "@/hooks/policies/securityPolicies/useSecurityPolicy";
...
function UpdateSecurityPolicy() {
  ...
  const { isLoading: isLoadingSecurityPolicy, securityPolicy } =
    useSecurityPolicy();
  const { isLoading: isUpdating, updatePolicy } = useUpdateSecurityPolicy();
  ...
  async function onSubmit({}, e) {
    ...
    updatePolicy({ policyId: securityPolicy.id, data: { content, visible } });
  }
  ...
  async function handleCancel() {
    setContent(securityPolicy?.content);
    setVisible(securityPolicy?.visible);
  }

  if (isLoadingSecurityPolicy) return <Spinner />;

  return (
    <>
      <Row type="horizontal">
        <Heading as="h1">Cập nhật chính sách bảo mật</Heading>
...
export default UpdateSecurityPolicy;
