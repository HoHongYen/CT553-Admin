...
import { useUpdateDeliveryPolicy } from "@/hooks/policies/deliveryPolicies/useUpdateDeliveryPolicy";
import { useDeliveryPolicy } from "@/hooks/policies/deliveryPolicies/useDeliveryPolicy";
...
function UpdateDeliveryPolicy() {
  ...
  const { isLoading: isLoadingDeliveryPolicy, deliveryPolicy } =
    useDeliveryPolicy();
  const { isLoading: isUpdating, updatePolicy } = useUpdateDeliveryPolicy();
  ...
  async function onSubmit({}, e) {
    ...
    updatePolicy({ policyId: deliveryPolicy.id, data: { content, visible } });
  }
  ...
  async function handleCancel() {
    setContent(deliveryPolicy?.content);
    setVisible(deliveryPolicy?.visible);
  }

  if (isLoadingDeliveryPolicy) return <Spinner />;

  return (
    <>
      <Row type="horizontal">
        <Heading as="h1">Cập nhật chính sách giao hàng</Heading>
...
export default UpdateDeliveryPolicy;
