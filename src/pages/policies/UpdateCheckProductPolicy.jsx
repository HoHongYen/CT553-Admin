...
import { useUpdateCheckProductPolicy } from "@/hooks/policies/checkProductPolicies/useUpdateCheckProductPolicy";
import { useCheckProductPolicy } from "@/hooks/policies/checkProductPolicies/useCheckProductPolicy";
...
function UpdateCheckProductPolicy() {
  const moveBack = useMoveBack();
  const { handleSubmit } = useForm();
  const { isLoading: isLoadingCheckProductPolicy, checkProductPolicy } =
    useCheckProductPolicy();
  const { isLoading: isUpdating, updatePolicy } = useUpdateCheckProductPolicy();
  ...
  async function handleCancel() {
    setContent(checkProductPolicy?.content);
    setVisible(checkProductPolicy?.visible);
  }

  if (isLoadingCheckProductPolicy) return <Spinner />;

  return (
    <>
      <Row type="horizontal">
        <Heading as="h1">Cập nhật chính sách kiểm hàng</Heading>
...
export default UpdateCheckProductPolicy;
