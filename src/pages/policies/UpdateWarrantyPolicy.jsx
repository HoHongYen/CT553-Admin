...
import { useUpdateWarrantyPolicy } from "@/hooks/policies/warrantyPolicies/useUpdateWarrantyPolicy";
import { useWarrantyPolicy } from "@/hooks/policies/warrantyPolicies/useWarrantyPolicy";
...
function UpdateWarrantyPolicy() {
  ...
  const { isLoading: isLoadingWarrantyPolicy, warrantyPolicy } =
    useWarrantyPolicy();
  const { isLoading: isUpdating, updatePolicy } = useUpdateWarrantyPolicy();
  ...
  async function onSubmit({}, e) {
    ...
    updatePolicy({ policyId: warrantyPolicy.id, data: { content, visible } });
  }
  ...
  async function handleCancel() {
    setContent(warrantyPolicy?.content);
    setVisible(warrantyPolicy?.visible);
  }

  if (isLoadingWarrantyPolicy) return <Spinner />;

  return (
    <>
      <Row type="horizontal">
        <Heading as="h1">Cập nhật chính sách bảo hành</Heading>
...
export default UpdateWarrantyPolicy;
