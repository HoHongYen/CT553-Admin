...
import { useHideSecurityPolicy } from "@/hooks/policies/securityPolicies/useHideSecurityPolicy";
import Table from "@/components/ui/Table";
import Tag from "@/components/ui/Tag";
import Button from "@/components/ui/Button";
import SpinnerMini from "@/components/ui/SpinnerMini";

function SecurityPolicyRow({ policy }) {
  const navigate = useNavigate();
  const { id: policyId, content, visible, createdAt, updatedAt } = policy;
  const { isLoading, toggleVisibility } = useHideSecurityPolicy();
      ...
      <Table.Row>
        ...
        <div className="flex justify-center gap-5">
          <Button
            ...
            onClick={() => navigate(`/chinh-sach-bao-mat/${policyId}`)}
          >
            <HiEye />
            Chi tiết
          </Button>
...
export default SecurityPolicyRow;
