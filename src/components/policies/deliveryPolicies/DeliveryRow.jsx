...
import { useHideDeliveryPolicy } from "@/hooks/policies/deliveryPolicies/useHideDeliveryPolicy";
import Table from "@/components/ui/Table";
import Tag from "@/components/ui/Tag";
import Button from "@/components/ui/Button";
import SpinnerMini from "@/components/ui/SpinnerMini";

function DeliveryPolicyRow({ policy }) {
  const navigate = useNavigate();
  const { id: policyId, content, visible, createdAt, updatedAt } = policy;
  const { isLoading, toggleVisibility } = useHideDeliveryPolicy();
      ...
      <Table.Row>
        ...
        <div className="flex justify-center gap-5">
          <Button
            ...
            onClick={() => navigate(`/chinh-sach-giao-hang/${policyId}`)}
          >
            <HiEye />
            Chi tiết
          </Button>
...
export default DeliveryPolicyRow;
