...
import { useHidePaymentPolicy } from "@/hooks/policies/paymentPolicies/useHidePaymentPolicy";
import Table from "@/components/ui/Table";
import Tag from "@/components/ui/Tag";
import Button from "@/components/ui/Button";
import SpinnerMini from "@/components/ui/SpinnerMini";

function PaymentPolicyRow({ policy }) {
  const navigate = useNavigate();
  const { id: policyId, content, visible, createdAt, updatedAt } = policy;
  const { isLoading, toggleVisibility } = useHidePaymentPolicy();
      ...
      <Table.Row>
        ...
        <div className="flex justify-center gap-5">
          <Button
            ...
            onClick={() => navigate(`/chinh-sach-thanh-toan/${policyId}`)}
          >
            <HiEye />
            Chi tiết
          </Button>
...
export default PaymentPolicyRow;
