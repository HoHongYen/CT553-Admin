...
import { useHideReturnPolicy } from "@/hooks/policies/returnPolicies/useHideReturnPolicy";
import Table from "@/components/ui/Table";
import Tag from "@/components/ui/Tag";
import Button from "@/components/ui/Button";
import SpinnerMini from "@/components/ui/SpinnerMini";

function ReturnPolicyRow({ policy }) {
  const navigate = useNavigate();
  const { id: policyId, content, visible, createdAt, updatedAt } = policy;
  const { isLoading, toggleVisibility } = useHideReturnPolicy();
      ...
      <Table.Row>
        ...
        <div className="flex justify-center gap-5">
          <Button
            ...
            onClick={() => navigate(`/chinh-sach-doi-tra/${policyId}`)}
          >
            <HiEye />
            Chi tiết
          </Button>
