...
import { useHideWarrantyPolicy } from "@/hooks/policies/warrantyPolicies/useHideWarrantyPolicy";
import Table from "@/components/ui/Table";
import Tag from "@/components/ui/Tag";
import Button from "@/components/ui/Button";
import SpinnerMini from "@/components/ui/SpinnerMini";

function WarrantyPolicyRow({ policy }) {
  const navigate = useNavigate();
  const { id: policyId, content, visible, createdAt, updatedAt } = policy;
  const { isLoading, toggleVisibility } = useHideWarrantyPolicy();
      ...
      <Table.Row>
        ...
        <div className="flex justify-center gap-5">
          <Button
            ...
            onClick={() => navigate(`/chinh-sach-bao-hanh/${policyId}`)}
          >
            <HiEye />
            Chi tiết
          </Button>
...
export default WarrantyPolicyRow;
